...
import Gargoyle.Redis (defaultRedis, runRedisLocalWithSubstitution)
...
main = do
  ...
  case args of
    dbPath:cmd:cmdArgs ->
      exitWith =<< runRedisLocalWithSubstitution defaultRedis dbPath cmd (\conn -> if null cmdArgs then [conn] else substArgs conn cmdArgs) Nothing
    _ -> do
      pname <- getProgName
